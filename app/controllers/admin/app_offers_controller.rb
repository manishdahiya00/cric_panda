module Admin
  class AppOffersController < AdminController
  layout  "admin"
  ...
  def create
    @app_offer = AppOffer.new(app_offer_params)
      if @app_offer.save
    redirect_to admin_app_offers_path, notice: "App offer was successfully created."
      else
  ...
  def update
      if @app_offer.update(app_offer_params)
        redirect_to admin_app_offers_path , notice: "App offer updated successfully."
      else
